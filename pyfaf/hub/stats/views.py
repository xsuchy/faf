...
def by_daterange(request, since, to,
    ...
    total_query = queries.query_history_sum(db, history=history)
    total = date_filter(total_query).one()[0]

    release_data = []

    for release in queries.query_releases(db):
        release_sum = queries.query_history_sum(
        ...
        comps = queries.query_report_count_per_component(
            db, release.opsys.name, release.version, history=history)
