...
class Bugzilla(object):
    ...
    def __init__(self, db, bz_url=pyfaf.config.get("bugzilla.url")):
        """
        Constructor expecting database instance `db` and
        valid bugzilla url `bz_url`.

        If `bz_url` is None, no bugzilla connection is initialized
    ...
    def query_bugs(self, to_date, from_date,
        ...
        logging.info("Fetching bugs modified between "
                     "{0} and {1}, offset is: {2}".format(from_date,
                                                          to_date, offset))
    ...
    def save_ccs(self, ccs, new_bug_id):
        ...
        for num, user_email in enumerate(ccs):
            ...
            cc = (
                self.db.session.query(RhbzBugCc)
                .join(RhbzUser)
    ...
    def save_history(self, events, new_bug_id):
        ...
        for num, event in enumerate(events):
            ...
            for change in event["changes"]:
                ...
                ch = (
                    self.db.session.query(RhbzBugHistory)
    ...
    def get_attachment(self, attachment_id):
        ...
        attachment = (
            self.db.session.query(RhbzAttachment)
        ...
        if not opsysrelease:
            if not self.add_opsysreleases:
                ...
                logging.error("Unable to find release '{0} {1}'. "
                              " Adding new one.".format(product, version))
    ...
    def set_whiteboard(self, bug_id, new_whiteboard, comment=None):
        """
        Set bug whiteboard of `bug_id` to `new_whiteboard`, optionally
        adding comment if `comment` string is present.
        """
